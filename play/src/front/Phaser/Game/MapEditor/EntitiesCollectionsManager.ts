...
export class EntitiesCollectionsManager {
    ...
    private async fetchRawCollection(url: string): Promise<EntityCollectionRaw> {
        console.log("🔍 DEBUG: Fetching entity collection from URL:", url);
        const response = await fetch(url);
        console.log("🔍 DEBUG: Response status:", response.status, response.statusText);
        if (!response.ok) {
            console.error("❌ DEBUG: Failed to fetch entity collection:", url, response.statusText);
            throw new Error(`Failed to fetch ${url}: ${response.statusText}`);
        }
        const data = await response.json();
        console.log("✅ DEBUG: Successfully loaded entity collection:", data);
        return data;
    }
