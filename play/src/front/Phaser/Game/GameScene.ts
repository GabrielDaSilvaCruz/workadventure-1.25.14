...
export class GameScene extends DirtyScene {
    ...
    constructor(private _room: Room, customKey?: string) {
        ...
        } else if (this.mapUrlFile) {
            // PATCH: Inject dummy WAM URL for on-premise/dev environments where .wam file might be missing
            console.warn("⚠️ DEBUG: WAM URL missing. Injecting dummy WAM URL.");
            this.wamUrlFile = this.mapUrlFile.replace(".tmj", ".wam").replace(".json", ".wam");
        }
    ...
    preload(): void {
        ...
        if (this.wamUrlFile) {
            ...
            // PATCH: Inject dummy WAM file for on-premise/dev environments where .wam file might be missing
            console.warn("⚠️ DEBUG: WAM file missing. Injecting dummy WAM file to enable Map Editor.");
            this.wamFile = {
                version: "1.0.0",
    ...
    public getCustomEntityCollectionUrl() {
        ...
        // PATCH: Fallback if wamUrlFile is undefined (direct TMJ load)
        const baseUrl = this.wamUrlFile ?? this.mapUrlFile ?? window.location.href;
        try {
            return new URL(mapStoragePath, baseUrl).toString();
        } catch (e) {
            console.warn("⚠️ DEBUG: Failed to construct custom entity URL, using window location");
            return new URL(mapStoragePath, window.location.href).toString();
        }
    }
    ...
    private async onMapLoad(data: any): Promise<void> {
        console.log("🔍 DEBUG: onMapLoad called");
        ...
        // PATCH: Load entityCollections from map properties if loading TMJ directly
        if (!this.wamFile && this.mapFile.properties) {
            const entityCollectionsProp = this.mapFile.properties.find((p: any) => p.name === "entityCollections");
            if (entityCollectionsProp) {
                console.log("🔍 DEBUG: Found entityCollections in map properties:", entityCollectionsProp.value);
                try {
                    if (typeof entityCollectionsProp.value !== 'string') {
                        throw new Error("entityCollections property value is not a string");
                    }
                    const collections = JSON.parse(entityCollectionsProp.value);
                    const mapBaseUrl = this.mapUrlFile.substring(0, this.mapUrlFile.lastIndexOf("/") + 1);

                    const collectionDescriptors = collections.map((c: any) => ({
                        url: new URL(c.url, mapBaseUrl).toString(),
                        type: "Default"
                    }));
                    ...
                    console.error("❌ DEBUG: Failed to parse entityCollections from map properties", e);
                }
            } else {
                console.log("⚠️ DEBUG: No entityCollections property found in map");
            }
        }
