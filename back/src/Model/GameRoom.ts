...
export class GameRoom implements BrothersFinder {
    ...
    public static async create(
        ...
        if (!wamUrl && mapDetails.mapUrl) {
            ...
            try {
                console.log("DEBUG: fetching WAM file from:", wamUrl);
                ...
                console.log("DEBUG: WAM file fetched successfully. mapUrl:", mapUrl);
            } catch (e) {
                console.error("DEBUG: Failed to fetch WAM file:", e);
                // Fallback to mapUrl if available, or rethrow?
                // If wamUrl was inferred, we might want to fallback to the original mapUrl if possible.
                // But here mapUrl is not set yet in this branch.
                // If we inferred wamUrl from mapUrl, we can just use the original mapUrl.
                if (mapDetails.mapUrl) {
                    console.log("DEBUG: Fallback to mapUrl from details:", mapDetails.mapUrl);
                    mapUrl = mapDetails.mapUrl;
                    wamFile = undefined;
                } else {
                    // If we inferred wamUrl, we derived it from a mapUrl.
                    // But getMapDetails returns mapUrl OR wamUrl.
                    // If it returned wamUrl, it means it didn't return mapUrl?
                    // My fix in getMapDetails sets BOTH mapUrl and wamUrl.
                    // So mapDetails.mapUrl should be set.
                    throw e;
                }
            }
        } else {
    ...
    private static async getMapDetails(roomUrl: string): Promise<MapDetailsData> {
        console.error("DEBUG: getMapDetails called with roomUrl:", roomUrl);
        if (!ADMIN_API_URL) {
            ...
            if (match && PUBLIC_MAP_STORAGE_URL) {
                ...
                console.log("DEBUG: getMapDetails static map detected. roomUrl:", roomUrl, "mapUrl:", mapUrl);
                if (mapUrl.endsWith(".tmj")) {
                    wamUrl = mapUrl.replace(".tmj", ".wam");
                    // If we have a WAM file, we can edit the map (at least partially, or we can read the settings)
                    canEdit = true;
                    console.log("DEBUG: inferred wamUrl:", wamUrl);
                } else if (mapUrl.endsWith(".wam")) {
                    wamUrl = mapUrl;
                    canEdit = true;
                    console.log("DEBUG: using wamUrl directly:", wamUrl);
                } else {
                    console.log("DEBUG: mapUrl does not end with .tmj or .wam");
                }
            }
            return {
                mapUrl,
